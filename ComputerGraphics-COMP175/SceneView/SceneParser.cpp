...
/*Added Stuff*/
void SceneParser::setupVec() {
    for (int i = 0; i < flattened_array.size(); i++) {
        delete flattened_array[i];
    }
    flattened_array.clear();
    Matrix matrix;
    flattenChild(getRootNode(), matrix);
    for (int i = 0; i < flattened_array.size(); i++) {
        matrix = flattened_array[i]->matrix;
        for (int j = 0; j < 16; j++) {
            cerr<<matrix.unpack()[j]<<" ";
        }
        cerr << endl;
    }
}
...
int SceneParser::flattenChild(SceneNode* node, Matrix matrix) {
    ...
    for (unsigned i = 0; i < length; i++) {
        ...
        switch (tform->type) {
            case TRANSFORMATION_TRANSLATE:
                matrix = matrix * trans_mat(tform->translate);
                break;
            case TRANSFORMATION_SCALE:
                matrix = matrix * scale_mat(tform->scale);
                break;
            case TRANSFORMATION_ROTATE:
                matrix = matrix * rot_mat(tform->rotate, tform->angle);
                break;
    ...
    FlatNode* curr = new FlatNode;
    curr->matrix = matrix;
    curr->node = node;
    int sum = 0;
    unsigned recurschild = node->children.size();
    for (unsigned i = 0; i < recurschild; i++) {
        cerr << recurschild<<"finding"<< i<<endl;

        sum += flattenChild(node->children[i], matrix);
    }
    curr->children = sum;
    flattened_array.push_back(curr);
    ++sum;
    return sum;
}
