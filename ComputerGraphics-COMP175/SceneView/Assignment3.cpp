...
void setupCamera();
void recurs_render(SceneNode* node);
...
void setupCamera()
{
	...
	camera->Reset(cameraData.aspectRatio);  //note that this is a new function. Be sure to set values for near and far plane!
...
void myGlutDisplay(void)
{
	...
	if (fillObj == 1) {
		...
		/*
		for (int i = 0; i < parser->flattened_array.size(); i++) {
			glPushMatrix();
			glMultMatrixd(parser->flattened_array[i]->matrix.unpack());
			SceneNode* node = parser->flattened_array[i]->node;
			int length = node->primitives.size();
			for (int j = 0; j < length; j++) {
				applyMaterial(node->primitives[j]->material);
				renderShape(node->primitives[j]->type);
			}
			glPopMatrix();
		}
		*/
...
void recurs_render(SceneNode* node) {
	glPushMatrix();
	SceneTransformation* tform;
	unsigned length = node->transformations.size();
	for (unsigned i = 0; i < length; i++) {
		tform = node->transformations[i];
		switch (tform->type) {
		case TRANSFORMATION_TRANSLATE:
			glMultMatrixd(trans_mat(tform->translate).unpack());
			break;
		case TRANSFORMATION_SCALE:
			glMultMatrixd(scale_mat(tform->scale).unpack());
			break;
		case TRANSFORMATION_ROTATE:
			glMultMatrixd(rot_mat(tform->rotate, tform->angle).unpack());
			break;
	...
	unsigned recurschild = node->children.size();
	length = node->primitives.size();
	for (int j = 0; j < length; j++) {
		if(!wireframe)
		applyMaterial(node->primitives[j]->material);
		renderShape(node->primitives[j]->type);
	}
	for (unsigned i = 0; i < recurschild; i++) {
		recurs_render(node->children[i]);
	}
	glPopMatrix();
}
void onExit()
